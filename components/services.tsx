  ...
  return (
    <section id="services" className="bg-[#222] p-10 space-y-10">
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {services.map((s) => (
          <div key={s.id} className="relative bg-white aspect-square">
            <Image src={s.img} alt={s.title} fill className="cover" />
            <div className="absolute bottom-0 bg-gradient-to-t w-full h-[50%] from-black to-transparent text-white flex flex-col justify-end p-4">
              <h2 className="text-xl">{s.title}</h2>
              <p className="opacity-50 text-sm">{s.description}</p>
            </div>
          </div>
        ))}
      </div>
